<x-app-layout>
    <style>
        .page-title { font-family:'Playfair Display',serif; color:#198754; font-weight:700; }
        .form-card { border:none; border-radius:20px; box-shadow:0 10px 30px rgba(0,0,0,0.05); background:#fff; border-top:5px solid #ffc107; }
        .form-label { font-weight:600; color:#495057; font-size:.9rem; margin-bottom:8px; display:flex; align-items:center; }
        .form-label i { margin-right:8px; color:#198754; }
        .form-control,.form-select { border-radius:12px; padding:12px 15px; border:1px solid #dee2e6; transition:all .3s; }
        .form-control:focus,.form-select:focus { border-color:#198754; box-shadow:0 0 0 0.25rem rgba(25,135,84,.1); }
        .btn-update { background:#198754; border:none; border-radius:12px; padding:12px 25px; font-weight:700; transition:.3s; }
        .btn-update:hover { background:#146c43; transform:translateY(-2px); box-shadow:0 5px 15px rgba(25,135,84,.3); }
        .current-file-box { background:#f8f9fa; border-radius:12px; padding:15px; border:1px dashed #dee2e6; margin-top:10px; }
        .section-divider { height:1px; background:#eee; margin:2rem 0; position:relative; }
        .section-divider span { position:absolute; top:-12px; left:20px; background:#fff; padding:0 10px; color:#999; font-size:.8rem; font-weight:600; text-transform:uppercase; }
    </style>
        ...
        <div class="card form-card">
            <div class="card-body p-4 p-md-5">
                ...
                <form action="{{ route('surat-masuk.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    {{-- ✅ METADATA INSTANSI DIPINDAH KE SINI (SEBELUM SUBMIT) --}}
                    @include('partials.metadata_instansi', ['pegawai' => $pegawai ?? collect(), 'data' => $data])
